...
public class ActivityFileController {
    ...
    ResponseEntity<PagedResponse<ActivityFile>> getActivityFile(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
    ...
    public ActivityFile getById(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
    ...
    public ResponseEntity<ActivityFile> createActivityFile(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
            @Valid @RequestBody(required = true) @Parameter(description="ActivityFile object - json") ActivityFile activityFile) throws BusinessLogicException, ExistentEntityException, BadRequestException, EntityNotExistentException, NoAccessGrantedException {
        try{
            ...
            if(activityFile.getUpdateUser()==null){
                activityFile.setUpdateUser(securityService.getUserByToken(token).getEmail());
            }
    ...
    public ResponseEntity<ActivityFile> updateActivityFile(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
                                                 @PathVariable(value = "id") @Parameter(description="ActivityFile Id - UUID") UUID activityFileId,
                                                 @Valid @RequestBody(required = true) @Parameter(description="ActivityFile object - json") ActivityFile activityFile, BindingResult bindingResult) throws BusinessLogicException, BadRequestException, EntityNotExistentException, ExistentEntityException, NoAccessGrantedException {
        try{
    ...
    @ApiResponses(value = {   
        @ApiResponse(responseCode = "202", description = "successful operation", content = @Content(array = @ArraySchema(schema = @Schema(implementation = ActivityFile.class)))),
        @ApiResponse(responseCode = "400", description = "bad request", content = @Content(array = @ArraySchema(schema = @Schema(implementation = ErrorDetails.class))))
    })
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
    public ResponseEntity<Boolean> deleteActivityFile(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
                                                 @PathVariable(value = "id") @Parameter(description="ActivityFile Id - UUID") UUID activityFileId,
        ...
        try{
            ...
            activityFileService.deleteActivityFile(activityFileId,updateUser);
    ...
    ResponseEntity<PagedResponse<ActivityFileLog>> getActivityFileLog(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
    ...
    public ActivityFileLog getLogById(@RequestHeader(value = "token", required = true) @Parameter(description="Sentinel Control Token - UUID") String token,
